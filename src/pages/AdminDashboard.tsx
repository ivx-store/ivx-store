...
import { LayoutGrid, Package, ExternalLink, Zap, Flame, Settings, TrendingUp, ClipboardList, LogOut, Loader2, MessageCircle, ArrowLeft, Users, FolderOpen, FileText } from "lucide-react";
import { AdminLogin } from "../components/admin/AdminLogin";
import { onAuthChange, logoutAdmin, getServices, getPackages, getOffers, getOrders, getMessages, getAllUsers, getCategories, getTestimonials, getFAQs, ADMIN_UID, formatTimestamp, type User, type OrderData } from "../lib/firebase";
...
export function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const loadAllCounts = async () => {
      ...
      try {
        const [services, packages, offers, orders, messages, usersData, categoriesData, testimonials, faqs] = await Promise.all([
          getServices(),
          getPackages(),
          getOffers(),
          getOrders(),
        ...
        setContentCnt(testimonials.length + faqs.length);
  ...
  const buttons: DashboardButton[] = [
    ...
    { id: "content", label: "المحتوى", icon: <FileText size={28} />, color: "#14b8a6", gradient: "linear-gradient(135deg, rgba(20,184,166,0.15) 0%, rgba(20,184,166,0.05) 100%)", path: "/admin/content", count: contentCnt, loading: countsLoading, description: "التعليقات والأسئلة الشائعة" },
